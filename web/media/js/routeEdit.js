...
function saveRoute() {
    ...
    $("#blocks li").each(function () {
        ...
        //if(data_row>newRoutes[data_col].stations.length){
        //    for(var j=0;j<stations.length;j++){
        //
        //
        //    }
        //   // newRoutes[data_col].stations.push()
        //}
        //alert(currentId);
        //newRoutes[data_col].stations[data_row].id = $(this).children('span').innerHTML;
        //i++;
    });
    //routs = newRoutes;
    //$("#blocks").empty();
    //$("#blocks").attr('style', null);
    //$("#grid").attr('class', 'gridster');
    loadData(newRoutes);
    // render();
}